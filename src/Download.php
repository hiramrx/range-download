...
class Download
{
    ...
    public function __construct($url, $saveAs, $options = array())
    {
        $this->url = $url;
        $this->client = new Client();

        $contentLength = $this->client->head($url, ['verify' => false])->getHeader("Content-Length");
        if (!$contentLength) {
            throw new \Exception('get object fail');
        }
        $this->totalSize = $contentLength[0];
...
//                    $request = $this->client->commandToRequestTransformer($command);
        ...
        $pool = new Pool($this->client, $uploadRequests(), [
